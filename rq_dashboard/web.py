...
def serialize_job(job):
    return dict(
        ...
        retry_intervals=job.retry_intervals if job.retry_intervals else [],
        retries_left=job.retries_left if job.retries_left else 0,
    )
...
def queues_overview(instance_number):
    r = make_response(
        render_template(
            ...
            instance_list=obfuscated_redis_urls(current_app.config.get("RQ_DASHBOARD_REDIS_URL")),
            queues=Queue.all(),
